$(document).ready(function(){
	$('#newsletter-signup').submit(function(){
		...
		//check the form is not currently submitting(2回目の送信は受け付けない)
		if($(this).data('formstatus') !== 'submitting'){
			...
			//send data to server for validation(サーバー側へリクエスト php/subscribe.php)
			$.ajax({
				url: formUrl,
				type: formMethod,
				data: formData,
				success:function(data){
					...
					//setup variables(データの受け取りと、判定用のインスタンス)
